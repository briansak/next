...
export function WebexDealSpacePicker({
  policyStatus,
}: {
  policyStatus: string;
}) {
  ...
  const allowlistedIds = new Set(allowlist.map((entry) => entry.spaceId));
  const displayError = error ?? spacesError;

  const loadAllowlist = useCallback(async () => {
    const res = await fetch("/api/integrations/webex/allowlist?purpose=DEAL");
    if (!res.ok) return;
  ...
  async function toggleSpace(space: WebexSpaceListItem) {
    ...
    const res = await fetch("/api/integrations/webex/allowlist", {
      ...
      body: JSON.stringify({
        ...
        purpose: "DEAL",
        dealLabel: labelDraft.trim() || undefined,
      }),
    });
    setBusyId(null);
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      setError(data.error ?? "Failed to update deal spaces");
      return;
    }
    await loadAllowlist();
    router.refresh();
  ...
  return (
    <div style={{ marginTop: "1.25rem" }}>
      ...
      >
        <h3 style={{ fontSize: "0.875rem", fontWeight: 600 }}>Deal spaces</h3>
      ...
      >
        Map Webex spaces for active customer or partner deals. Summaries appear on
        My Priorities — separate from day-job and technology spaces.
      ...
      >
        Deal label (optional, applied when adding a space)
      </label>
      <input
        type="text"
        placeholder="e.g. WWT Security POV"
        value={labelDraft}
        onChange={(event) => setLabelDraft(event.target.value)}
        list="deal-label-suggestions"
        style={{
          width: "100%",
          background: "var(--bg)",
      ...
      />
      <datalist id="deal-label-suggestions">
            ...
            return (
              <li
                ...
                style={{
                  ...
                  borderBottom: "1px solid var(--border)",
                  background: selected ? "rgba(232, 197, 91, 0.08)" : "transparent",
                }}
              >
                <div style={{ minWidth: 0 }}>
                  <p
